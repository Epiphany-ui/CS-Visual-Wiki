    ...
    def construct(self):
        ...
        def update_rectangles(mob, dt):
            ...
            for i in range(n):
                ...
                height = curve(left_x)
                # 计算矩形在场景中的宽度和高度
                p_left = axes.c2p(left_x, 0)
                p_right = axes.c2p(right_x, 0)
                p_top = axes.c2p(0, height)
                rect_width = p_right[0] - p_left[0]
                rect_height = p_top[1] - p_left[1]
                rect = Rectangle(
                    width=rect_width,
                    height=rect_height,
                    fill_color=YELLOW,
                ...
                # 矩形左下角对齐到 (left_x, 0) 的场景坐标
                bottom_left = axes.c2p(left_x, 0)
                rect.move_to(bottom_left, aligned_edge=DL)
                mob.add(rect)

        # 添加 updater（不在 updater 内部重复添加）
