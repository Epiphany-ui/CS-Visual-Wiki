-- ============================================================
-- Manim 动画生成系统 - 完整数据库初始化脚本
-- 数据库：manim_ai
-- 说明：覆盖 8 大业务模块（用户、首页、百科、模板、沙箱、
--       社区画廊、个人中心、搜索），共 12 张表
-- ============================================================
...
-- 对应接口：5.3 / 6.3 / 6.2（template.generate / sandbox.render）
-- 设计原因：追踪每次 AI 渲染请求，支撑异步任务状态轮询。
-- user_id 关联用户，status 区分处理中/成功/失败，
-- video_path 存渲染结果文件路径，error_log 记录失败日志。
-- ============================================================
...
-- ============================================================
-- 3. 百科分类表：学科分类导航
-- 对应接口：2.2 / 4.1（home.category / knowledge.category.list）
-- 设计原因：首页分类导航 + 百科分类列表页的数据基础，
-- sort_order 控制显示顺序，icon 存分类图标。
-- ============================================================
